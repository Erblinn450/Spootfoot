...
export default function InviteLanding() {
  ...
  // Charger automatiquement si un token est fourni par navigation
  React.useEffect(() => {
    if (initialToken) {
      load();
    }
  ...
  const accept = async () => {
    ...
    if (r.status === 200) {
      const data = await r.json();
      const text = `Merci, vous êtes inscrit. acceptedCount=${data.acceptedCount}`;
      setMessage(text);
      Alert.alert('Inscription', text);
    } else {
      const txt = 'Malheureusement, le créneau est plein';
      setMessage(txt);
